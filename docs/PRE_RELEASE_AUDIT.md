...
**Tanda P2 — 4ª parte** (commit `ff3b4a2`): TEST-001 — **Vitest + 40 tests unitarios** de las funciones PURAS que deciden pagos (`money.ts` 17 incl. el invariante "suma de las 3 partes = lo que pagó el cliente", `geo.ts` 10, `delivery-pricing.ts` 13) + **CI** (`.github/workflows/ci.yml`: typecheck + lint + test en cada push/PR). Antes un `git push` no ejecutaba ninguna aserción. Sin credenciales, reproducible, riesgo cero.
- **P2 restantes (no hechos):** AUTHZ-004 (squatting de unique_ids → mover reserva a Admin SDK, refactor del signup), residual de AUTHZ-002 (spam de notif interna falsa → Admin SDK + App Check), **TEST-002** (test de integración del webhook de MP — necesita mockear la API de MP o un emulador).
- **Estado P2: 7 resueltos.** Quedan los 2 refactors de escritura-cliente→Admin-SDK (AUTHZ-004 + residual AUTHZ-002) y TEST-002.
